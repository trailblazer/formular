...
module Formular
  ...
  class Element
    ...
    self.default_hash = {} #I actually don't want to merge default classes...
    ...
    def normalize_attributes(options={})
      ...
      default_options = default_attributes.select { |k,v| default_attributes.delete(k) || true  if option_key?(k) }
    ...
    def get_defaults
      attrs = {}
      self.class.default_hash.each { |k,v| attrs[k] = (v.is_a?(Symbol) && self.respond_to?(v)) ? self.send(v) : v }
