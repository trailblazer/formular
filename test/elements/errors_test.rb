...
describe "errors module" do
  ...
  let(:builder) { Formular::Builders::Basic.new(
      errors: { body: ["This really isn't good enough!"], public: ["What's wrong with you??!"] },
      elements: { wrapped_input: WrappedInput, error_wrapper: ErrorWrapper }
    )
  }
  ...
  describe "builder hasn't got attribute error" do
    ...
    it "when custom error should be true" do
      element = builder.wrapped_input(:some_other_attribute, error: "I'm an error")
      element.has_errors?.must_equal true
    end
  end
